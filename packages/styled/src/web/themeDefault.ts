...
export const colors = {
	...
	status1: 'hsl(198, 99%, 45%)',
	status2: 'hsl(143, 68%, 44%)',
	status3: 'hsl(25, 99%, 54%)',
	status4: 'hsl(0, 93%, 58%)',
	status5: 'hsl(259, 86%, 53%)',
	status6: 'hsl(299, 80%, 59%)',
	status7: 'hsl(210, 11%, 71%)',
	// STATUS END
...
export const theme = {
	...
	gradient60x40: ({ color, angle }: GradientInterface) => {
		const angleParse = `${angle ?? 180}deg`;
		const colorStart = theme.transparentize({ color: color ?? theme.primary, amount: 0.4 });
		const colorEnd = theme.transparentize({ color: color ?? theme.primary, amount: 0.6 });
		return `linear-gradient(${angleParse}, ${colorStart} 0%, ${colorEnd} 100%)`;
	},
	gradient0x70: ({ color, angle }: GradientInterface) => {
		const angleParse = `${angle ?? 180}deg`;
		const colorStart = color ?? theme.primary;
		const colorEnd = theme.transparentize({ color: color ?? theme.primary, amount: 0.3 });
		return `linear-gradient(${angleParse}, ${colorStart} 0%, ${colorEnd} 100%)`;
	},
	gradient10x5: ({ color, angle }: GradientInterface) => {
		const angleParse = `${angle ?? 180}deg`;
		const colorStart = theme.transparentize({ color: color ?? theme.primary, amount: 0.9 });
		const colorEnd = theme.transparentize({ color: color ?? theme.primary, amount: 0.95 });
		return `linear-gradient(${angleParse}, ${colorStart} 0%, ${colorEnd} 100%)`;
	},
